...
Route::namespace ('Admin')->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::middleware('admin')->group(function () {
        ...
        Route::prefix('domain')->name('domain.')->group(function () {
            Route::get('/all', 'ManageDomainController@allDomain')->name('all');
            Route::get('/pending', 'ManageDomainController@pendingDomain')->name('pending');
            Route::get('/approved', 'ManageDomainController@approvedDomain')->name('approved');
            Route::get('/finished', 'ManageDomainController@finishedDomain')->name('finished');
            Route::get('/rejected', 'ManageDomainController@rejectedDomain')->name('rejected');
            Route::get('/sold', 'ManageDomainController@soldDomain')->name('sold');
            Route::get('/view/{id}', 'ManageDomainController@viewDomain')->name('view');
            Route::post('/approve/confirm', 'ManageDomainController@domainApprove')->name('approve');
            Route::post('/reject/confirm', 'ManageDomainController@domainReject')->name('reject');
            Route::post('/verification/send', 'ManageDomainController@fileSend')->name('send');

            Route::get('all-bids/{id}', 'ManageDomainController@allBids')->name('bids');
            Route::get('reported-bids', 'ManageDomainController@reportedBids')->name('bid.reported');

            Route::get('conversations/{bid_id}', 'ManageDomainController@viewConversation')->name('bid.conversation.view');
            Route::post('send/message', 'ManageDomainController@sendMessage')->name('bid.conversation.send-message');
            Route::post('/bid/report/action/{id}', 'ManageDomainController@reportAction')->name('bid.report.action');

            Route::get('bid/conversation/attachment/download/{value}/{id}','ManageDomainController@downloadAttachment')->name('conversation.attachment.download');

        });
        // Domain Extension
        Route::prefix('domain')->group(function () {
            Route::get('/extension/index', 'DomainExtensionController@index')->name('extension.domain.index');
            Route::post('/extension/store/{id?}', 'DomainExtensionController@store')->name('extension.domain.store');
        });

        // Make Notice
        Route::get('notice-for-buyer', 'GeneralSettingController@noticeForBuyer')->name('notice.index');
        Route::post('notice-for-buyer', 'GeneralSettingController@noticeUpdate')->name('notice.update');
...
Route::name('user.')->prefix('user')->group(function () {
    Route::middleware('auth')->group(function () {
        ...
        Route::middleware(['checkStatus'])->group(function () {
            ...
            // Deposit
            Route::any('/deposit-money', 'Gateway\PaymentController@deposit')->name('deposit');
            ...
            Route::any('my-deposits', 'UserController@depositHistory')->name('deposit.history');
            ...
            Route::get('/auction/comments/{id}', 'AuctionController@showAllComment')->name('auction.comments');

            // Contact Seller - User
            Route::get('contact-seller/user/{id}/{name}', 'ContactSellerController@contactSeller')->name('contact.seller.user');
            Route::post('contact-seller/user/message/{id}', 'ContactSellerController@messageToSeller')->name('contact.seller.message.user.send');
            Route::get('contact-seller/user-conversations/{id}', 'ContactSellerController@allMessages')->name('contact.seller.user.message.all');

            // Contact Seller - Seller
            Route::get('contact-seller/owner/messages/{id}', 'ContactSellerController@showAllMessage')->name('contact.seller.owner.message');
            Route::get('contact-seller/owner/show-messages/{domainId}/{userId}', 'ContactSellerController@showMessages')->name('contact.seller.owner.message.view');
            Route::get('contact-seller/owner/conversations/{id}/{slug}', 'ContactSellerController@conversationView')->name('contact.seller.owner.conversations');
            Route::post('contact-seller/owner/message/{domainId}/{userId}', 'ContactSellerController@messageReply')->name('contact.seller.owner.message.send');

            Route::get('attachment/download/{value}/{id}', 'ContactSellerController@attachmentDownload')->name('contact.seller.attachment.download');


            Route::get('bid/{id}/{name}', 'BidController@makeBid')->name('make.bid');
            Route::post('bid/store/{id}', 'BidController@bidStore')->name('make.bid.store');

            Route::post('/comment/post', 'BidController@commentPost')->name('comment.post');
            Route::post('/comments/reply/{id}', 'BidController@commentReply')->name('comments.reply');

            Route::get('bids/history', 'BidController@myBids')->name('bids.history');
            Route::get('bids/winner', 'BidController@bidWinner')->name('bids.winner');

            Route::get('auction/bids/{id}', 'BidController@bidDetail')->name('bid.details');
            Route::post('/bid/approve/{id}', 'BidController@bidApprove')->name('bid.approve');

            Route::get('auction/bid/conversation/{id}/{domain_id}','BidController@conversation')->name('bid.conversation');
            Route::post('auction/bid/credential/{id}','BidController@giveCredential')->name('bid.credential');
            Route::post('auction/bid/report/{id}','BidController@reportOnBid')->name('bid.report');
            Route::post('auction/bid/complete/{id}','BidController@completeBid')->name('bid.complete');
            Route::get('bid/conversation/show/{id}','BidController@conversationShow')->name('bid.conversation.show');
            Route::post('auction/bid/conversation/store/{bid_id}/{domain_id}','BidController@conversationStore')->name('bid.conversation.store');

            Route::get('bid/conversation/attachment/download/{value}/{id}','BidController@attachmentDownload')->name('bid.conversation.attachment.download');
...
Route::get('seller/profile/{id}/{name}', 'SiteController@sellerProfile')->name('seller.profile');
// Comment
Route::get('comment/show/{id}', 'SiteController@commentShow')->name('comment.show');

// domain
Route::get('domains/all', 'DomainController@domains')->name('domain.list');
Route::get('domain/filter', 'DomainController@domainFilter')->name('domain.filter');
Route::get('filter/domain/pagination', 'DomainController@domainFilter');
Route::get('detail/{id}/{name}', 'DomainController@domainDetail')->name('domain.detail');
Route::get('domain/search', 'DomainController@domainSearch')->name('domain.search');
Route::get('pages/{uniqueId}', 'SiteController@redirectUrl')->name('url.redirect');
